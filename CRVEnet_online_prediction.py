...
def prediction(model, training_channels, target_channels, camera_channels, ntime_sensor, dataloader, device, output_folder):
    k = 1
    for input_, camera_data_, sensor_data_, target_ in dataloader:
        inputs = input_[:, training_channels, :, :, :]
        camera_data = camera_data_[:, camera_channels, :, :]
        ...
        output_path = output_folder + str(k) + '/'
        os.makedirs(output_path, exist_ok=True)

        np.save(output_path + 'inputs_' + str(k) + '.npy', input_.cpu().detach().numpy())
        np.save(output_path + 'camera_data_embedding' + str(k) + '.npy', camera_data_.cpu().detach().numpy())
        np.save(output_path + 'sensor_data_embedding' + str(k) + '.npy', sensor_data_.cpu().detach().numpy())
        np.save(output_path + 'outputs_' + str(k) + '.npy', outputs.cpu().detach().numpy())
        np.save(output_path + 'targets_' + str(k) + '.npy', target_.cpu().detach().numpy())
        k = k + 1
...
for power in powers:
    for speed in speeds:
        ...
        for stdpos in std_pos:
            dir_path = f'data/train_data_std_dt6.4e-4_rve1.6_0.4_0.1/power{power}_speed{speed}_stdpos{stdpos}'
...
dataloader = DataLoader(combined_dataset, batch_size=1, shuffle=False, num_workers=0,  pin_memory=True)
...
model_folder = "Validation_2Dto3D/std_test5/model.pt"
output_folder = "Online_prediction_2Dto3D/std_test5/"

model.load_state_dict(torch.load(model_folder))

prediction(model, training_channels, target_channels, cdata_channels, ntime_sensor, dataloader, 'cuda', output_folder)
